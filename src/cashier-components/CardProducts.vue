<template>
   <div class="card shadow mb-4">
      ...
      <div class="card-body">
         ...
         <routerLink v-if="partner_id" class="btn btn-warning font-weight-bold rounded-pill px-3 shadow mr-2"
            :to="'/detail-store-product/' + products.id + '/' + partner_id">Check Product</routerLink>
         <button @click="addToCart" class="btn btn-primary font-weight-bold rounded-pill px-3 shadow">
...
<script>
   ...
   export default {
      ...
      methods: {
         ...mapActions(["getPromos", "getCart", "getPartners", "getPartner"]),
         ...
         addToCart() {
            const checkPartner = this.cart.store.id
            if (checkPartner !== this.partner_id) {
               swal({
                     title: "Kamu sudah belanja di toko lain!",
                     text: "Apakah kamu ingin ganti toko?",
                     icon: "warning",
                     buttons: true,
                     dangerMode: ['Ganti','Tidak'],
                  })
                  .then((willDelete) => {
                     if (willDelete) {
            ...
            const selected = checkCart.cart_items.find(
               (cart) => cart.product.id == this.products.id
            );

            if (this.isCartItems == true && selected != undefined) {
               selected.qty += 1;
               ...
               axios
                  .put("http://localhost:3000/cart/", checkCart)
                  .then(() => {
                     swal("Jumlah produk " + this.products.name + " di tambah 1 pcs", {
                        icon: "success",
                     });
                  })
               ...
               axios
                  .post("http://localhost:3000/cart/", this.addProduct)
                  .then(() => {
