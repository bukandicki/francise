      ...
      <div class="row">
        ...
        <div class="col-md-7">
          ...
          <h3 class="d-block mb-5" v-if="partner && partner.store_promo">
            <strike class="text-danger small">Rp{{ product.price }}</strike>
            Rp{{ product.price - (product.price * (parseInt(promo_partner.amount) / 100)) }}
          </h3>
          <h2 class="d-block mb-5" v-else>
            <strike class="text-danger small">Rp{{ product.price }}</strike>
            <strong class="ml-2"
              >Rp{{ product.price - (product.price * (parseInt(promo_id.amount) / 100)) }}</strong
            >
          </h2>
          ...
          <button class="btn btn-primary rounded-pill mt-2" @click="addToCart">
            Add To Cart
          </button>
        </div>
      </div>
  ...
  methods: {
    ...
    addToCart() {
      ...
      if (this.isCartItems == true && selected != undefined) {
        ...
        axios
          ...
          .then(() => {
            swal("Jumlah produk " + this.product.name + " di tambah " + this.qty + " pcs", {
              icon: "success",
            });
          })
    ...
    addQty() {
      if (this.qty < this.product.stock) {
        ...
        this.$toast.success("Maaf stock " + this.product.name + " terbatas ", {
          type: "error",
          position: "top-right",
          duration: 3000,
          dismissible: true,
