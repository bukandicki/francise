   ...
   <section class="home-cashier-page">
      <div class="container pt-5">
         ...
         <div class="option-partner-page w-100 mb-5 d-flex justify-content-between">
            <input class="form-control w-25" @keyup="searchStore" v-model="search" type="text"
               placeholder="Search partner..." />
         </div>
         <div class="row">
            <div class="col-lg-12 text-center pt-5" v-if="store.length < 1">
               <h1 class="mt-5">
                  Toko
                  <strong class="text-primary">{{ search }}</strong> tidak
                  di temukan.
               </h1>
            </div>
            <div class="col-md-4 col" v-for="item in store" :key="item.id" v-else>
               <div class="card shadow border-0 mb-4">
                  ...
                  <div class="card-body">
                     ...
                     <router-link class="btn btn-primary mt-3 rounded-pill px-3" :to="'/store-profile/' + item.id">Visit
                        Store</router-link>
                  </div>
               </div>
            </div>
